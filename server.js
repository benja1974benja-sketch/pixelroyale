...
// Velocidad del jugador por tick.
// 11 px cada 50 ms = aproximadamente 220 px/segundo.
...
Esto evita que el cliente y el servidor
se peleen por la posición del jugador.
*/
...
function updatePlayer(p, data) {
  ...
  // Evitar velocidades superiores al máximo.
  ...
  p.x = Math.max(
    20,
    Math.min(WORLD - 20, p.x)
  );

  p.y = Math.max(
    20,
    Math.min(WORLD - 20, p.y)
  );
      ...
      case "startMatch":
        ...
        if (!partyCanStart(player)) {
          send(ws, {
            type: "error",
            message:
              "Solo el líder puede iniciar y todos deben estar listos."
          });
